...
function Process-FilesInAppFolder {
    ...
    if ($isUpload -and $null -ne $sourceID -and $sourceID -ne "") {
        Upload-ToSailPoint -newFile $newFile -sourceID $sourceID -clientURL $SettingsObject.tenant -fileUploadUtility $SettingsObject.FileUploadUtility -ClientID $SettingsObject.ClientID -ClientSecret $SettingsObject.ClientSecret -AppLogFilePath $AppLogFilePath
    }
...
$requiredSettings = @('AppFolder', 'ExecutionLogDir', 'tenant', 'FileUploadUtility', 'ClientID', 'ClientSecret')
...
$executionLogFilePath = Join-Path -Path $SettingsObject.ExecutionLogDir -ChildPath $executionLogFileName
...
# Main Execution
$AppFolders = Get-ChildItem -Path $targetDirectory -Directory
$startTime = Get-Date
